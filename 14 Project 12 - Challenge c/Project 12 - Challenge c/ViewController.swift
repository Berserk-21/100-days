...
class ViewController: UITableViewController {
    ...
    private var currentWordKey: String = "currentWordKey"
    private var currentWord: String = ""
    private var alreadyUsedWordKey: String = "alreadyUsedWordKey"
    ...
    private func saveUsedWords(key: String, usedWords: [String]) {
        
        let userDefault = UserDefaults.standard
        userDefault.set(usedWords, forKey: key)
    }
    
    private func startGame() {

        let userDefault = UserDefaults.standard
        if let currentWord = userDefault.object(forKey: currentWordKey) as? String {
            title = currentWord
            self.currentWord = currentWord
            
            if let alreadyUsedWords = userDefault.object(forKey: currentWord) as? [String] {
                self.usedWords = alreadyUsedWords
            }
            ...
            if isPossible(word: lowerAnswer) {
                if isOriginal(word: lowerAnswer) {
                    if isReal(word: lowerAnswer) {
                        ...
                        saveUsedWords(key: currentWord, usedWords: usedWords)
